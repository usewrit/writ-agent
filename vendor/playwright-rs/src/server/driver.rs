...
pub fn get_driver_executable() -> Result<(PathBuf, PathBuf)> {
    // writ-agent patch: honor the runtime env overrides FIRST. The agent points
    // PLAYWRIGHT_NODE_EXE/CLI_JS (or PLAYWRIGHT_DRIVER_PATH) at patchright's
    // STEALTH driver; without this reorder the bundled VANILLA 1.60 driver
    // (priority #1 below) would shadow it on a dev build, leaving Runtime.enable
    // on → instant anti-bot detection + console-event-flood latency. Both env
    // helpers verify the paths exist, so a stale/unset override cleanly falls
    // through to the bundled driver (unchanged behavior when no override is set).
