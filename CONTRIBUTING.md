...
**Regenerate from a tree you have actually built, and ignore whitespace-only diffs reported by the
advisory CI job.** cargo-about reads each crate's license text out of `registry/src/`, which only a
build populates — `cargo fetch` leaves `.crate` archives unextracted, and for anything missing
cargo-about quietly substitutes the generic SPDX text. That makes the output unstable on a cold
runner, which is why the CI job is advisory; the blocking check lives in the publish pipeline, where
it runs after a full build.
